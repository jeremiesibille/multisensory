...
def run_stimulus(params,setup):
    ...
    opto_conditions = params['stimulus']['opto_conditions']
    ...
    for t in range(n_trials):
        ...
        opot_conditions_sequence = np.concatenate((opot_conditions_sequence,tmp_opto),axis=0)
    ...
    opto_start_sec = params['stimulus']['opto_start']
    opto_start_frame = np.round(opto_start_sec*refreshrate).astype('int')
    
    opto_duration = params['stimulus']['opto_duration']
    opto_duration_frames = np.round(opto_duration*refreshrate).astype('int')
    
    opto_stop_frame = opto_start_frame + opto_duration_frames
...
#    stimulus_changes_variable = params['stimulus']['local_type']
    ...
    for ind in range(len(orientation_sequence)):
        ...
        opto_tmp = opot_conditions_sequence[ind]
        ...
        for frameN in range(stimulus_duration_frames):     # 1 frame = 8ms, range = duration of 1 frame/image  , 500 / 120 =>500 = 4 sec, 200/120-->1.6s
            ...
            if frameN == opto_start_frame:
                if opto_tmp == 'red':
                    trigger.OpotRedStart()
                    print(str(triggercount)+'- opto red')
                if opto_tmp == 'blue':
                    trigger.OpotBlueStart()
                    print(str(triggercount)+'- opto blue')
                if opto_tmp == 'noopto':
                    print(str(triggercount)+'- no opto')
                    
            # opto stop
            if frameN == opto_stop_frame:
                if opto_tmp == 'red':
                    trigger.OpotRedStop()
                    print(str(triggercount)+'- Stop opto red')
                if opto_tmp == 'blue':
                    trigger.OpotBlueStop()
                    print(str(triggercount)+'- Stop opto blue')
#                if opto_tmp == 'noopto':
#                    print(str(triggercount)+'- no opto')
#            
        # after the stimulus we have to deactivate the opto anyway
        if opto_tmp == 'red':
            trigger.OpotRedStop()
        if opto_tmp == 'blue':
            trigger.OpotBlueStop()
